...
that a preset expresses which signals matter rather than acting as a granularity
...
$D_{\max}$ is divided into $\lceil d / D_{\max} \rceil$ equal parts. Equal division is
used rather than truncation, so that no negligible trailing fragment is produced. A
...
$20$\,s.
Duration bounds are deliberately not applied in \texttt{interval} mode, since the
...
4--8 frames retained, thinned against evenly spaced points in time; VLM
...
identifiers assigned by overlap tracking across every candidate frame, then read
...
is post-processed with \texttt{locations}, holding the timestamp and box in every frame
they appear in, and a \texttt{people\_count} is recorded. \\
\texttt{object\_\allowbreak detection} &
Shares the same YOLO11n pass rather than repeating it; frames annotated with cyan
boxes carrying no labels; up to 6 frames; VLM at 1280\,px, \texttt{detail=high}. &
...
debugging. \\
\texttt{ocr} &
EasyOCR detection stage only, since its recognition stage
...
Where an analyzer's gate finds nothing, meaning no person above the area threshold, no
object detection and no text region, the chunk yields no output and no API call is made.
...
\texttt{description}, \texttt{people}, \texttt{actions} and \texttt{objects}, of which
\texttt{combined} must always be present. A space is omitted rather than emitted
...
\midrule
\texttt{stats} & None & Arithmetic only &
...
\texttt{speaker\_stats} & \texttt{diarization} & Arithmetic only &
...
\texttt{summary} & None & LLM &
A hierarchy built by repeated halving. Depth is derived from chunk count, taking enough
halvings for a leaf to cover at most five chunks and bounded to between three and six
tiers, so that leaf granularity does not vary with recording length. Leaves are summarised
...
\texttt{ner} & None & GLiNER \texttt{urchade/\allowbreak gliner\_small-v2.1}, zero-shot, threshold $0.5$ &
...
Per person: first and last sighting, total span, observed presence summed over
...
Measured on this footage, provably different people, meaning people co-visible within
one chunk and so unable to be the same, reached $0.941$ similarity, while known-same pairs sat
...
\begin{enumerate}[leftmargin=1.6em]
  ...
  \item \textbf{Within-chunk merge.} Observations in the same chunk above a similarity
        of $0.95$, which is above the measured co-visible ceiling, are merged first. The
...
since two shopping carts genuinely look alike where two people rarely wear the same
...
\textbf{Score threshold.} Cosine similarity always ranks something, so a query for content
...
\textbf{Detail level.} Three levels are offered: \texttt{minimal}, \texttt{standard}
...
hint, which is a short description of what that aggregate is good for, written in the
vocabulary a question would use. The hints are embedded and compared against the question,
